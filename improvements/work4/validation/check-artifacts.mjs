...
const reportPath = join(root, "AUDIT_V4_REPORT.md");
...
// The 18 medium+ findings filed as issue templates.
const findingIds = Array.from({ length: 18 }, (_, i) => `WORK4-${String(i + 1).padStart(3, "0")}`);
...
if (!existsSync(issuesDir)) {
  ...
  for (const file of files) {
    ...
    const idMatch = frontmatter[1].match(/^finding-id:\s*"?(WORK4-\d{3})"?/m);
...
console.log("work4 audit artifacts are structurally valid");
